...
test('@TC09 Merge Previous segments', async ({ page }) => {
  ...
  await stepPause(page, 'Ready to merge the second segment with previous');
  await workspace.mergeSecondSegmentWithPrevious();
  await stepPause(page, 'Merge previous completed', 1500);
});
