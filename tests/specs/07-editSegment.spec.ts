...
test('@TC07 Edit segment transcript', async ({ page }) => {
  ...
  await stepPause(page, 'Workspace opened');
  const editedText = await workspace.editFirstSegment(run.editedSegmentSuffix);
  updateRunState({ editedText });
  await stepPause(page, 'Segment text edited and persisted', 1500);
});
