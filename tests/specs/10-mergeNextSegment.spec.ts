...
test('@TC10 Merge Next segments', async ({ page }) => {
  ...
  await stepPause(page, 'Ready to merge the first segment with next');
  await workspace.mergeFirstSegmentWithNext();
  await stepPause(page, 'Merge next completed', 1500);
});
